...
- `app.js`: 카메라, 하단 번호 이미지 OCR, 검색, 최근 기록, 컬렉션 로직
...
1. 카메라 가이드 안에 카드 하단 번호가 오도록 맞춥니다.
2. 앱은 카드 외곽선 감지를 기다리지 않고 가이드 안 하단 번호 이미지를 바로 잘라 OCR합니다.
3. 사진 스캔을 선택하면 찍은 이미지에서 하단 번호 영역을 즉시 OCR합니다.
4. `087/063`, `90/063`, `090/063` 같은 번호 패턴만 유효하게 인정합니다.
