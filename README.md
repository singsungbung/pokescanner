...
- OpenCV.js: 카메라 화면 속 카드 외곽선 추적

첫 로딩, 첫 OCR, 첫 카드 외곽선 추적은 인터넷 연결과 라이브러리 초기화 때문에 느릴 수 있습니다.

## 스캔 방식

1. 카메라 가이드 안에 카드 이름과 하단 번호가 오도록 맞춥니다.
2. 앱은 카드 crop 이미지를 먼저 만들고, `data/visual-index.js`가 있으면 이미지 특징값을 먼저 비교합니다.
3. 카드 외곽선이 흔들리는 동안에는 내부 점수만 조용히 갱신하고, 안정화된 뒤 이미지 기반 후보를 띄우거나 lock-on합니다.
4. lock-on 뒤에는 OCR을 계속 반복하지 않고, 같은 카드인지/새 카드인지 visual feature만 빠르게 검토합니다.
5. lock-on 카드가 후보에서 밀리면 강한 새 후보는 빠르게 전환하고, 애매하면 lock-on을 풀어 재스캔합니다.
6. 카드가 화면에서 사라진 것으로 보이면 lock-on을 풀고 새 카드 대기 상태로 돌아갑니다.
7. OCR은 번호/이름이 보일 때만 보조 검증으로 사용합니다.
8. visual index가 비어 있으면 OCR/fuzzy 후보가 임시 fallback으로 동작합니다.
9. 사진 스캔을 선택하면 찍은 이미지에서 카드 crop, 이름 영역, 번호 영역을 즉시 분석합니다.
10. 숫자 OCR에서 자주 틀리는 `O/I/S/Z/B/G` 등은 숫자로 보정합니다.
11. `O5I/O92`, `05I/092`, `51/92`, `087/O63`, `090063` 같은 결과도 DB 번호와 유사도 비교를 합니다.
12. 카드 이름은 일본어, 한글, 영어 이름을 모두 비교합니다.
13. 점수가 애매하면 바로 실패하지 않고 top 3 후보를 보여줍니다.
14. 최근 후보를 최대 10번까지 모아 같은 후보가 반복되면 신뢰도를 높입니다.
15. 인식 실패 시 `001/063` 또는 첫 번째 카드로 대체하지 않습니다.
...
스캐너 가이드는 OpenCV.js가 로드되면 카드 외곽선을 따라 움직이고, 실패하면 카드 비율 중앙 crop으로 fallback합니다. `스캔 진단` 패널에서 현재 crop과 visual 후보 점수를 확인할 수 있습니다. visual 후보는 1등 점수뿐 아니라 2등과의 차이도 확인해 너무 애매한 경우 바로 lock-on하지 않습니다.
