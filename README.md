...
3. 번호가 읽히면 번호로 먼저 DB를 찾고, 번호가 흔들리면 카드 이름 OCR로 보조 매칭합니다.
4. 사진 스캔을 선택하면 찍은 이미지에서 이름/번호 영역을 즉시 OCR합니다.
5. `087/063`, `90/063`, `090/063`, `090063` 같은 번호 패턴만 유효하게 인정합니다.
6. 현재 선택된 세트 DB에 없는 번호는 `DB에 없는 카드`로 표시합니다.
7. 인식 실패 시 `001/063` 또는 첫 번째 카드로 대체하지 않습니다.
