...
5. 카드가 화면에서 사라진 것으로 보이면 lock-on을 풀고 새 카드 대기 상태로 돌아갑니다.
6. OCR은 번호/이름이 보일 때만 보조 검증으로 사용합니다.
7. visual index가 비어 있으면 OCR/fuzzy 후보가 임시 fallback으로 동작합니다.
8. 사진 스캔을 선택하면 찍은 이미지에서 카드 crop, 이름 영역, 번호 영역을 즉시 분석합니다.
9. 숫자 OCR에서 자주 틀리는 `O/I/S/Z/B/G` 등은 숫자로 보정합니다.
10. `O5I/O92`, `05I/092`, `51/92`, `087/O63`, `090063` 같은 결과도 DB 번호와 유사도 비교를 합니다.
11. 카드 이름은 일본어, 한글, 영어 이름을 모두 비교합니다.
12. 점수가 애매하면 바로 실패하지 않고 top 3 후보를 보여줍니다.
13. 최근 후보를 최대 10번까지 모아 같은 후보가 반복되면 신뢰도를 높입니다.
14. 인식 실패 시 `001/063` 또는 첫 번째 카드로 대체하지 않습니다.
